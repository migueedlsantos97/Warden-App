...
- **Fotos de jugadores**: los 3 tienen foto real en `assets/` (recortadas de las tarjetas estilo NBA que pasó el usuario).
- **Nombre completo (apellido + apodo)**: en la fila de Tabla y en el detalle del jugador se muestra grande el apellido y arriba, chico y en mayúscula, el apodo — igual que en las tarjetas de referencia. Murrey no tiene apellido, así que se muestra solo su nombre grande sin la línea chica. Miguelón → apellido "Jackson"; Mojarrita → apellido "Porraz". En el resto de la app (wizard, historial, logros, H2H) se sigue usando el apodo solo, para no generar confusión tipo "¿quién es Jackson?" en medio de cargar un partido — si se quiere extender a algún otro lugar puntual, es un cambio chico.
